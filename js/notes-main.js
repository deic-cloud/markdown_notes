...
(function () {
	...
	function metaSorted() {
		...
		arr.sort(function (a, b) {
			...
			if (key === 'title') { r = (a.title || '').localeCompare(b.title || ''); }
				...
				r = (va !== '' && vb !== '' && !isNaN(na) && !isNaN(nb)) ? (na - nb) : va.localeCompare(vb);
			}
			return r * dir || ((b.modified || 0) - (a.modified || 0));
		});
		return arr;
	}
	function sortByHeader(key) {
		if (state.metaSort.key === key) { state.metaSort.dir = state.metaSort.dir === 'asc' ? 'desc' : 'asc'; }
		else { state.metaSort = { key: key, dir: 'asc' }; }
		renderList();
	}
	...
	function ensureEditor() {
		...
		installFind(mde.codemirror, mde.codemirror.getWrapperElement().parentNode, FIND_LABELS);
	}
	...
	var FIND_LABELS = {
		placeholder: t('markdown_notes', 'Find in this note'), none: t('markdown_notes', 'No matches'),
		of: t('markdown_notes', '{n} of {total}'), prev: t('markdown_notes', 'Previous (Shift+Enter)'),
		next: t('markdown_notes', 'Next (Enter)'), close: t('markdown_notes', 'Close (Esc)'),
	};
	...
	function stampRow(s) {
		...
		} else if (s.verified === 'not-pinned') {
			state1.className += ' notes-stamp-bad';
			state1.textContent = t('markdown_notes', 'Not from an accepted authority');
		} else if (s.verified === 'outside-window') {
			state1.className += ' notes-stamp-bad';
			state1.textContent = t('markdown_notes', 'Dated outside the accepted period');
